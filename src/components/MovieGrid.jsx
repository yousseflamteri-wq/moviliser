    ...
    <div className="flex flex-col gap-8 pb-12 overflow-x-hidden pt-20">
      ...
      <div className="relative flex w-full max-w-4xl mx-auto h-[400px] sm:h-[500px] items-center justify-center">
        {heroMovies.map((movie, index) => {
          ...
          if (index === heroIdx) {
            position = 'opacity-100 scale-100 z-20 translate-x-0 shadow-2xl shadow-red-900/20'; 
          } else if (index === (heroIdx - 1 + heroMovies.length) % heroMovies.length) {
            position = 'opacity-40 scale-90 z-10 -translate-x-[55%] sm:-translate-x-[65%]'; 
          ...
          return (
            <div 
              key={movie.id}
              className={`absolute transition-all duration-700 ease-in-out w-[70%] sm:w-[50%] max-w-[320px] aspect-[2/3] rounded-xl overflow-hidden bg-zinc-900 cursor-pointer ${position}`}
            ...
            >
              ...
              {/* Anchored Bottom Overlay with Title & Button */}
              <div className="absolute inset-x-0 bottom-0 flex flex-col items-center justify-end bg-gradient-to-t from-black via-black/90 to-transparent p-4 pt-16 pb-6 text-center">
                ...
                {/* Button only renders on the active center card */}
                {index === heroIdx && (
                  <button 
                    onClick={(e) => {
                      e.stopPropagation(); // Prevents double-firing the card click
                      onSelect(movie);
                    }}
      ...
      {/* Dynamic Genre Rows */}
      {/* Added mt-12 for proper spacing from the hero section */}
        ...
        {Object.entries(categorized).map(([genre, rowMovies]) => (
          <div key={genre} className="flex flex-col">
            ...
            <div className="mb-3 flex items-center justify-between">
              {/* Added capitalize to fix lowercase markdown errors */}
